...
export default function Index({ cities, filters }) {
    ...
    const runSearch = (e) => {
        e?.preventDefault();
        router.get(route('admin.cities.index'), { search: q || undefined }, { preserveState: true, replace: true });
    };
    ...
    const destroyRow = (id) => {
        ...
        if (!confirm('Delete this city? Trade shows linked to it will block deletion.')) {
            return;
        }
    ...
    return (
        <AdminLayout title="Cities">
            ...
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                <div>
                    <h2 className="text-lg font-semibold text-slate-900">City listing</h2>
                    <p className="mt-1 text-sm text-slate-500">Master data from `citytables`.</p>
                </div>
                            ...
                            {rows.length === 0 ? (
                                ...
                                rows.map((row, idx) => (
                                    <tr key={row.id} className="hover:bg-slate-50/80">
                                        <td className="whitespace-nowrap px-4 py-3 text-slate-600">{serialNumber(cities, idx)}</td>
                                        <td className="px-4 py-3 font-medium text-slate-900">{row.name}</td>
                                        <td className="px-4 py-3 text-slate-700">{row.countryname ?? '—'}</td>
                                        ...
                                        <td className="whitespace-nowrap px-4 py-3">
                                            <div className="flex items-center gap-2">
                                                <Link
                                                    href={route('admin.cities.edit', row.id)}
            ...
            <TablePagination pagination={cities} />
        </AdminLayout>
    );
}
